...
class SimClient:
    ...
    async def handle_ws_message(self, msg: Dict) -> None:
        ...
        if msg_type == "hello":
            self.sync_offset(msg.get("data", {}).get("server_time"))
            return
    ...
    def sync_offset(self, server_time: Optional[int]) -> None:
        if not server_time:
            return
        self.server_offset_ms = int(server_time) - now_ms()

    async def api_request(self, method: str, path: str, json_body: Optional[Dict] = None) -> Tuple[int, Dict]:
    ...
    def make_sign(self, round_id: int, drop_id: int, client_ts: int) -> str:
        ...
        return hmac.new(self.sign_secret.encode(), msg, hashlib.sha256).hexdigest()
    ...
    async def send_click(self, drop: Drop) -> None:
        ...
        await self.api_request("POST", "/api/game/click", payload)
        self.click_count += 1
